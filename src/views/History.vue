        ...
        <v-row align="center" justify="center">
          <v-col v-for="historyItem in history"  v-bind:key="historyItem.time"  sm="6" cols="12" md="4" lg="4">
            <v-card  elevation="5" class="mx-6 my-1 rounded-lg">
              <v-btn icon style="position: absolute; right:0">
              ...
              <v-card-actions>
                <v-spacer/>
                <span class="grey--text" style="font-size: 0.8em">{{new Date(historyItem.time).toLocaleDateString()
                + ' @ ' + new Date(historyItem.time).toLocaleTimeString()}}</span>
              </v-card-actions>
            </v-card>
          </v-col>
        </v-row>
...
export default {
  ...
  mounted(){
    ...
    this.$root.$on('new-message', (messageObj) => {this.history.push(messageObj)})
  },
  data: function(){
    return {
      history: []
